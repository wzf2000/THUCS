...
if __name__ == '__main__':
	...
	if args.is_train:
		...
		plt.plot(range(1, args.num_epochs + 1), val_losses, label="test loss")
		plt.legend()
		...
		plt.plot(range(1, args.num_epochs + 1), val_accs, label="test accuracy")
		plt.legend()
		...
		plt.savefig('__'.join([key + '=' + str(value) for key, value in out_dict.items()]) + '.png')
	else:
		mlp_model = Model()
		mlp_model.to(device)
		model_path = os.path.join(args.train_dir, 'checkpoint_%d.pth.tar' % args.inference_version)
