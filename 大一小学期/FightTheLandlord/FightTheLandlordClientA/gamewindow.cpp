...
GameWindow::GameWindow(QTcpServer *_listen, ClientWindowA *cw, QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::GameWindow)
    , clientWindow(cw)
    , listenSocket(_listen)
    ...
    , req(Play::nullPlay)
{
    ...
    readWriteSocketB = listenSocket->nextPendingConnection();
    readWriteSocketC = listenSocket->nextPendingConnection();

    connect(readWriteSocketB, &QTcpSocket::readyRead, this, &GameWindow::readPackageB);
    connect(readWriteSocketC, &QTcpSocket::readyRead, this, &GameWindow::readPackageC);

    PackageManager::instance().sendPackage(this, readWriteSocketB, 3);
    QThread::sleep(1);
    PackageManager::instance().sendPackage(this, readWriteSocketC, 3);
...
void GameWindow::readPackageB()
{
    int flag;
    do
    {
        int out;
        while (!(flag = PackageManager::instance().readPackage(this, readWriteSocketB, out)))
            QThread::sleep(1);
        if (!certain)
        {
            if (out == 6)
            {
                disconnect(readWriteSocketB, &QTcpSocket::readyRead, this, &GameWindow::readPackageB);
                disconnect(readWriteSocketC, &QTcpSocket::readyRead, this, &GameWindow::readPackageC);
                std::swap(readWriteSocketB, readWriteSocketC);
                connect(readWriteSocketB, &QTcpSocket::readyRead, this, &GameWindow::readPackageB);
                connect(readWriteSocketC, &QTcpSocket::readyRead, this, &GameWindow::readPackageC);
            }
            certain = true;
        ...
        if (out < 2)
        {
            gaming = (gaming + 1) % 3;
            this->out = out;
            PackageManager::instance().sendPackage(this, readWriteSocketC, 1);
            if (out == 1)
            {
                pre = (gaming + 2) % 3;
                if (gaming % 3 == LOCALID)
            ...
            if (gaming == randBegin)
            {
                ...
                pre = -1;
                for (int i = 51; i < 54; i++)
                    landlordBoard->cards.push_back(v[i]);
                landlordBoard->update();

                PackageManager::instance().sendPackage(this, readWriteSocketB, 10);
                PackageManager::instance().sendPackage(this, readWriteSocketC, 10);
                ...
                if (gaming == LOCALID)
                {
                    ...
                    for (int i = 51; i < 54; i++)
                        board->manager->cards.push_back(v[i]);
                    rest[gaming] += 3;

                    std::sort(board->manager->cards.begin(), board->manager->cards.end(), [](const Card *l, const Card *r) { return *l > *r; });
                    board->update();
                ...
                {
                    PackageManager::instance().sendPackage(this, gaming == 1 ? readWriteSocketB : readWriteSocketC, 7);
                    rest[gaming] += 3;
        ...
        if (out > 1 && out < 4)
        {
            ...
            else
            {
                ...
                if (!rest[pre])
                {
                    ...
                    ui->playButton->setEnabled(true);
                    ui->notOutButton->setEnabled(true);
                    PackageManager::instance().sendPackage(this, readWriteSocketC, 2);
                    continue;
                }
            }
            PackageManager::instance().sendPackage(this, readWriteSocketC, 2);
        ...
        if (out == 8)
        {
            ...
            if (ifRestart == 7)
            {
                PackageManager::instance().sendPackage(this, readWriteSocketB, 4);
                PackageManager::instance().sendPackage(this, readWriteSocketC, 4);
                start();
            }
        }
        if (out == 14)
        {
            if (ifRestart & 2)
                ifRestart ^= 2;
            PackageManager::instance().sendPackage(this, readWriteSocketC, 8);
            QMessageBox::warning(this, QStringLiteral("连接断开"), QStringLiteral("连接已断开，请点击退出！"));
        }
    } while(flag == 2);
}

void GameWindow::readPackageC()
{
    int flag;
    do
    {
        int out;
        while (!(flag = PackageManager::instance().readPackage(this, readWriteSocketC, out)))
            QThread::sleep(1);
        if (!certain)
        {
            if (out == 5)
        ...
        if (out < 2)
        {
            gaming = (gaming + 1) % 3;
            this->out = out;
            PackageManager::instance().sendPackage(this, readWriteSocketB, 1);
            if (out == 1)
            {
                pre = (gaming + 2) % 3;
                if (gaming % 3 == LOCALID)
        ...
        if (out > 1 && out < 4)
        {
            ...
            else
            {
                ...
                if (!rest[pre])
                {
                    ...
                    ui->playButton->setEnabled(true);
                    ui->notOutButton->setEnabled(true);
                    PackageManager::instance().sendPackage(this, readWriteSocketB, 2);
                    continue;
                }
            }
            PackageManager::instance().sendPackage(this, readWriteSocketB, 2);
        ...
        if (out == 15)
        {
            if (ifRestart & 4)
                ifRestart ^= 4;
            PackageManager::instance().sendPackage(this, readWriteSocketB, 8);
            ui->playButton->setDisabled(true);
            QMessageBox::warning(this, QStringLiteral("连接断开"), QStringLiteral("连接已断开，请点击退出！"));
        }
    } while (flag == 2);
...
void GameWindow::start()
{
    ...
    for (int i = 1; i <= 13; i++)
    {
        v.push_back(new Card('C', i, this));
        v.push_back(new Card('D', i, this));
        v.push_back(new Card('H', i, this));
        v.push_back(new Card('S', i, this));
    }
    v.push_back(new Card('B', this));
    v.push_back(new Card('R', this));

    char *p = new char;
    srand((unsigned long long)time(0) + (unsigned long long)p);
    delete p;
    std::random_shuffle(v.begin(), v.end());
    board->manager->cards.clear();
    ...
    PackageManager::instance().sendPackage(this, readWriteSocketB, 5);
    PackageManager::instance().sendPackage(this, readWriteSocketC, 6);
    ...
    getLandlord = false;
    landlord = -1;
    randBegin = pre = gaming = rand() % 3;
    board->status = (LOCALID == gaming);
    update();

    PackageManager::instance().sendPackage(this, readWriteSocketB, 9);
    PackageManager::instance().sendPackage(this, readWriteSocketC, 9);
...
void GameWindow::on_playButton_clicked()
{
    ...
    if (landlord == -1)
    {
        ...
        if (gaming == randBegin)
        {
            ...
            for (int i = 51; i < 54; i++)
                board->manager->cards.push_back(v[i]), landlordBoard->cards.push_back(v[i]);
            landlordBoard->update();
